...
app.use((err, req, res, next) => {
  if (err.type === 'entity.too.large') {
    console.error(`Payload too large: ${req.method} ${req.url} - ${req.headers['content-length']} bytes`);
    return res.status(413).json({ error: 'Payload too large', limit: '500mb', actual: req.headers['content-length'] });
  }
  next(err);
});
